...
def UPLOAD(update, context):
    ...
    if os.path.isfile(ID):
        ...
        if "openload" in url or "oload" in url:
            ...
            sent_message.edit_text("Openload artıq mövcud deyil")
            return
            ...
            print("Dropbox linki yüklənməyə başladı : {}".format(
                url.split("/")[-1]))
            sent_message.edit_text(TEXT.DP_DOWNLOAD)
            # filename = wget.download(url)
            filename = wget_dl(str(url))
            print("Yükləmə tamamlandı : {}".format(filename))
           ...
           # Here IS Mega Links stuffs
            ...
            try:
                print("Aşağı qiymətləndirmə başladı")
                ...
                print("Tam Mega yüklənir :", filename)
                sent_message.edit_text(TEXT.DOWN_COMPLETE)

                DownloadStatus = True
            except Exception as e:
                print("Meqa aşağı salma xətası :", e)
                sent_message.edit_text("Meqa Yükləmə Xətası !!")

        else:
            try:
                filename = url.split("/")[-1]

                print("Endirmə başladı : {}".format(url.split("/")[-1]))
                ...
                if TEXT.DOWN_TWO:
                    ...
                    try:
                        sent_message.edit_text(
                            "Yükləyici 1 Xətası:{} \n\n Yükləyici 2: Yükləmə başladı...".format(e))
                        ...
                        print(e)
                        sent_message.edit_text(
                            "Yükləmə xətası :{}".format(e))
                        DownloadStatus = False
                else:
                    print(e)
                    sent_message.edit_text("Yükləmə xətası :{}".format(e))
                    DownloadStatus = False
        ...
        if "error" in filename:
            ...
            sent_message.edit_text("Yükləmə xətası !! ")
        ...
        try:
            ...
            if DownloadStatus:
                ...
                try:
                    ...
                    print("səhv kod : UPX11", e)
                    sent_message.edit_text("Yükləmə baş tutmadı :{}".format(e))
                else:
            ...
            print("Xəta kodu UXP12", e)
            if DownloadStatus:
                sent_message.edit_text("Yükləmə baş tutmadı : {}".format(e))
                try:
                    os.remove(filename)
                except Exception as e:
                    print("Xəta kodu UXP13", e)
            else:
                sent_message.edit_text("Yükləmə baş tutmadı :", e)
